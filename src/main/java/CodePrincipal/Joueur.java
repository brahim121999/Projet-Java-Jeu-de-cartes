...
public class Joueur {
	...
	private List<Integer> liste_score;	//Liste des scores du joueur par semaine, de la semaine 0 � la semaine actuelle
	...
	public Joueur(Jeu jeu, String nom, String prenom, boolean poste, List<Integer> liste_equipe, List<Integer> liste_score) {
		...
		//On ajoute le joueur � la liste des joueurs du jeu
		jeu.getListe_Joueur().add(this);
		
		//On ajoute le joueur � la liste des joueurs de ses �quipes
		...
		for (int i = 0; i < equipes.size(); i++) {
			...
			for (int j = 0; j < liste_equipe.size(); j++) {
				...
				if (equipe.getId_equipe() == id_equipe) {
					//On verifie que le joueur n'est pas d�j� pr�sent dans la liste des joueurs de l'�quipe
